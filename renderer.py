...
def render(img, proj_matrix, ref_U, eyemask, facemask, opts):
    ...
    frontal_sym, sym_weight = mysoftSymmetry(img, frontal_raw, ref_U, in_proj, ind_frontal, bg_mask, facemask[:, 0],
                                             eyemask, opts)
...
def mysoftSymmetry(img, frontal_raw, ref_U, in_proj, \
    ...
    if eyemask is not None and opts.getboolean('renderer', 'symmetry'):  # one side is ocluded
        ...
        synth_frontal_acc = cv2.GaussianBlur(synth_frontal_acc, (ksize_acc, ksize_acc), 30.,
